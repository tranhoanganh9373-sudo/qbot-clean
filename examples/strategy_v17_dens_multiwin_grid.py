"""参数化 v17 grid sweep — DoubleEnsemble + 多窗口 ensemble 版.

每月训练 12 月 + 24 月 两个 train 窗口的 DEns, score 等权平均 → pred.
动机: 短窗口适应近期, 长窗口跨 regime 稳定. 加权平均期望抑制单窗口噪声.

设计:
  PRED_CACHE = v17_dens_multiwin_predictions.parquet (独立 cache).
  每月跑 2 次 fit (12m + 24m), ~80s/月 (vs 单窗口 ~40s). 60月 wall ≈ 80 min.

输出:
  examples/v17_dens_multiwin_<tag>_stats.csv

Run:
  python examples/strategy_v17_dens_multiwin_grid.py --k 8 --drop 2 \\
      --tag multiwin_k8d2_60m --first-test 2021-05 --last-test 2026-04 --months 60
"""
...
PRED_CACHE = ROOT / "data_cache" / "v17_dens_multiwin_predictions.parquet"
...
TRAIN_WINDOWS = [12, 24]  # 多窗口 ensemble: 训 12 + 24 月, score 等权平均


def _train_one_window(test_month_start, n_months: int) -> pd.Series:
    """单窗口训练, 返回 pred Series."""
    test_start = month_start(test_month_start)
    test_end = month_end(test_month_start)
    train_start = month_start(test_month_start - relativedelta(months=n_months))
...
def get_pred_for_month(test_month_start):
    ...
    if cached is not None:
        ...
        return cached
    # 多窗口 ensemble: 每个 train 窗口跑一次, score 等权平均
    preds = []
    for win in TRAIN_WINDOWS:
        print(f"  [multiwin] train_months={win} ...", flush=True)
        preds.append(_train_one_window(test_month_start, win))
    # 等权平均
    avg_pred = sum(preds) / len(preds)
    _pred_cache[key] = avg_pred
    _persist_pred(key, avg_pred)
    return avg_pred
...
def main():
    ...
    out_csv = OUT_DIR / f"v17_dens_multiwin_{args.tag}_stats.csv"
    ...
    mm["drop"] = args.drop
    mm["tag"] = args.tag
    print(f"\n=== SUMMARY (DEnsemble + Multi-Window {TRAIN_WINDOWS}) tag={args.tag} K={args.k} D={args.drop} ===")

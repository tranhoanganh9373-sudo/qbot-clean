"""train24 (v19.1 production) sub-period regime validation 2019-2020.

目的: 验证 train24 在跨 regime (2019 科技反弹 + 2020 COVID+V) 是否仍 robust.
策略: 严格不重训, 只读 cached predictions (v17_dens_train24_predictions.parquet).
新 cache 由 strategy_v17_dens_grid.py 重训生成 (--first-test 2019-01 --last-test 2020-12).

2018 跳过原因: baidu_kline 在 2018 有负复权价格 (17 stocks, 860 records),
2018 月度回测会出现 artifact (e.g. +555%). 详见 memory:project-2018-kline-corrupt.
...
Run:
  python examples/strategy_train24_subperiod_2019_2020.py
"""
...
def main():
    ...
    print(f"\n[pre-check] target sub-period: 2019-01 ~ 2020-12 ({len(target_months)} months)")
    ...
    out_csv = OUT_DIR / "v17_dens_train24_subperiod_2019_2020_stats.csv"
    ...
    p_all = filter_period(df, "2019-01", "2020-12", "sub-period total", "mixed")
    ...
    # 后 24m 用 csv 直接算 (复用 v17_dens_train24_stats.csv if exists, else skip)
    prod_csv = OUT_DIR / "v17_dens_train24_stats.csv"
    if prod_csv.exists():
        prod_df = pd.read_csv(prod_csv)
        # 取 2024-05 ~ 2026-04 即最后 24 月
        last24 = prod_df[(prod_df["month"] >= "2024-05") & (prod_df["month"] <= "2026-04")]
        if len(last24) > 0:
            mm24 = annualize_metrics(last24["abs_ret_%"])
            print(f"{'2024-05~2026-04 (24m)':<26} {f'{len(last24)}/24':>7} {'known':<18} "
                  f"{mm24['cum_%']:>8.2f} {mm24['ann_%']:>7.2f} {mm24['sharpe']:>7.2f} "
                  f"{mm24['mdd_%']:>8.2f} {mm24['calmar']:>7.2f} {mm24['win_%']:>7.2f}")

    # 判定
    print("\n=== Verdict ===")
    valid_yearly = [p for p in (p19, p20) if p is not None]
    n_yearly_valid = len(valid_yearly)
    n_yearly_target = 2
    all_periods_positive = all(p["ann_%"] > 0 for p in valid_yearly) if valid_yearly else False
    has_big_drop = any(p["cum_%"] < -20 for p in valid_yearly) if valid_yearly else False
    ...
    if n_yearly_valid < n_yearly_target:
        ...
        print("  [A. ROBUST] 2019+2020 both positive + sub-period Calmar > 1.0 + no big drop")
        ...
        print("  [C. FAIL] 2019/2020 large loss > 20% or Calmar < 0.5 — train24 是 2021-2026 friendly 产物")
    else:

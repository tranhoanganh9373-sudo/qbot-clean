"""v19.5 Sidecar — train24 + 球队硬币因子 (team_coin) overlay.

参考: hugo2046/QuantsPlaybook 仓库 "球队硬币" 笔记本.

因子逻辑:
    intraday_ret  = close / open - 1
    overnight_ret = open  / prev_close - 1
    team_coin     = intraday × overnight  (日内 × 隔夜 同向 = 双向认同;异向 = 分歧)
    team_coin_5d  / 20d = rolling sum

文献 ICIR -4.73 (横截面正向高 → 反转后期望负收益). **反向使用 (sign -1)** 在 sidecar:

    final_score = z(train24_pred) - λ_5  * z(team_coin_5d)
                                   - λ_20 * z(team_coin_20d)
...
Run:
  .venv/bin/python examples/strategy_v19_5_team_coin.py
"""
...
ADJ_PRED = ROOT / "data_cache" / "v19_5_team_coin_predictions.parquet"
OUT_IS_GRID = ROOT / "examples" / "v19_5_team_coin_is_grid.csv"
OUT_OOS_STATS = ROOT / "examples" / "v19_5_team_coin_oos_stats.csv"
...
IS_COMBOS = [
    ("c1_tc5_l010",       0.10, 0.00, 0.00),
    ("c2_tc5_l020",       0.20, 0.00, 0.00),
    ("c3_tc5_l030",       0.30, 0.00, 0.00),
    ("c4_tc20_l010",      0.00, 0.10, 0.00),
    ("c5_tc20_l020",      0.00, 0.20, 0.00),
    ("c6_tc20_l030",      0.00, 0.30, 0.00),
    ("c7_tc5tc20_l020",   0.10, 0.10, 0.00),
    ("c8_tc5tc20_l030",   0.15, 0.15, 0.00),
    ("c9_tc5margin_l020", 0.10, 0.00, 0.10),
]
...
def build_margin_panel() -> pd.DataFrame:
    ...
    Returns (datetime, instrument, z_margin_5d_chg).
    """
...
def build_adjusted_predictions(tc_panel: pd.DataFrame,
                               margin_panel: pd.DataFrame,
                               lam_5: float, lam_20: float, lam_m: float,
                               label: str) -> Path:
    """final = z(pred) - lam_5 * z(tc5) - lam_20 * z(tc20) - lam_m * z(margin)."""
    pred = pd.read_parquet(ORIG_PRED)
    pred["z_pred"] = pred.groupby("datetime")["score"].transform(_zscore_cs)
    merged = pred.merge(tc_panel, on=["datetime", "instrument"], how="left")
    if lam_m > 0:
        merged = merged.merge(
    ...
    merged[["z_team_coin_5d", "z_team_coin_20d"]] = merged[
        ["z_team_coin_5d", "z_team_coin_20d"]
    ].fillna(0.0)
    merged["final_score"] = (
        merged["z_pred"]
        - lam_5  * merged["z_team_coin_5d"]
        - lam_20 * merged["z_team_coin_20d"]
    ...
    print(f"  [adj] {label} λ=(tc5={lam_5},tc20={lam_20},m={lam_m}) "
...
def main() -> int:
    print("=" * 70)
    print("Phase 4 Sidecar v19.5 — train24 + 球队硬币因子")
    print("=" * 70)
    ...
    for label, lam_5, lam_20, lam_m in IS_COMBOS:
        print(f"\n  --- {label}  λ=(tc5={lam_5}, tc20={lam_20}, m={lam_m}) ---")
        build_adjusted_predictions(
            tc_panel, margin_panel, lam_5, lam_20, lam_m, label,
        )
    ...
    best_5 = float(best["lam_tc5"])
    best_20 = float(best["lam_tc20"])
    best_m = float(best["lam_margin"])
    print(f"\n[lock] best IS: {best_label}  "
          f"λ=(tc5={best_5}, tc20={best_20}, m={best_m})  "
    ...
    print(f"  locked config: {best_label} "
          f"λ=(tc5={best_5}, tc20={best_20}, m={best_m})")
    build_adjusted_predictions(
        tc_panel, margin_panel, best_5, best_20, best_m, best_label,
    )
    ...
    print(f"\nv19.5 (team_coin sidecar) OOS [{best_label}]:")
    ...
    if oos_stats["calmar"] > v194_calmar:
        print("\n[verdict] OOS Calmar > v19.4 → 推荐升级 production v19.5")
    else:
        print("\n[verdict] OOS Calmar <= v19.4 → abort, team_coin sidecar 不优于 margin")

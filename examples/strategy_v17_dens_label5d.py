...
class Alpha158Label5D(Alpha158):
    """Alpha158 + 5D forward return label (替代默认 1D).
    默认: Ref($close, -2) / Ref($close, -1) - 1 (~1日 fwd return)
    5D:   Ref($close, -6) / Ref($close, -1) - 1 (5日 fwd return, 匹配 K=8 D=2 ~4天持仓)
    """
...
PRED_CACHE = ROOT / "data_cache" / "v17_dens_label5d_predictions.parquet"  # 5D label 独立 cache
...
def main():
    ...
    # pred cache: 不同 market + train_months + model 各用独立 (label5d 已锁前缀)
    cache_suffix = "_label5d"
    if MARKET != "csi300":
    ...
    out_csv = OUT_DIR / f"v17_dens_label5d_{args.tag}_stats.csv"
    ...
    mm["drop"] = args.drop
    mm["tag"] = args.tag
    print(f"\n=== SUMMARY (DEnsemble + 5D Label) tag={args.tag} K={args.k} D={args.drop} ===")
    print(pd.Series(mm).to_string())
    print(f"\n输出: v17_dens_label5d_{args.tag}_stats.csv")

"""参数化 v17 grid sweep — DoubleEnsemble + adaptive K (基于 cross-sectional score std).

动机: model 置信度高 (今日 top scores std 大) → K=3 集中
      model 置信度低 (score 分布平坦, std 小) → K=8 分散

实现: 每日 trading 前算 top-32 candidates score std (z-score 标准差),
      std_z > 1.0 → K=3 (信号强, 集中)
      std_z < 0.5 → K=8 (信号弱, 分散)
      0.5 ~ 1.0 之间线性插值

数据: 复用 v17_dens_predictions.parquet (60月 DEns pred), 不重训, 仅改 trading sim.

输出:
  examples/v17_dens_adaptive_<tag>_stats.csv

Run:
  python examples/strategy_v17_dens_adaptive_grid.py --k 8 --drop 2 \\
      --tag adaptive_k8d2_60m --first-test 2021-05 --last-test 2026-04 --months 60
"""
...
def realistic_window(test_month_start, proxy, with_regime: bool = True):
    ...
    for di, td in enumerate(test_dates):
        ...
        # adaptive K: 算 当日 top-32 candidates 的 score z-std, 高 → 缩 K, 低 → 扩 K
        # K_NORMAL=8 base; std_z > 1.0 → K=3 集中; std_z < 0.5 → K=8 分散
        try:
            day_scores = pred_unstacked.loc[td].dropna().sort_values(ascending=False)
            top_pool = day_scores.head(topk * CANDIDATE_POOL_MULTIPLIER)
            all_mean = day_scores.mean()
            all_std = day_scores.std()
            if all_std > 0 and len(top_pool) > 3:
                # z-std of top pool relative to全 cross-section
                top_z = (top_pool - all_mean) / all_std
                top_z_std = float(top_z.std())
                # 线性插值: 0.5 → K_NORMAL(8), 1.0 → K_concentrated(3)
                k_concentrated = 3
                if top_z_std >= 1.0:
                    topk = k_concentrated
                elif top_z_std <= 0.5:
                    ...
                    # 在 0.5 ~ 1.0 之间线性
                    frac = (top_z_std - 0.5) / 0.5
                    topk = max(k_concentrated, int(round(K_NORMAL - frac * (K_NORMAL - k_concentrated))))
        except Exception:
            pass
...
def main():
    ...
    # 不同 train_months 用独立 pred cache 避免污染默认 (12 月) cache
    if TRAIN_MONTHS != 12:
        PRED_CACHE = ROOT / "data_cache" / f"v17_dens_train{TRAIN_MONTHS}_predictions.parquet"
    ...
    out_csv = OUT_DIR / f"v17_dens_adaptive_{args.tag}_stats.csv"
    ...
    mm["drop"] = args.drop
    mm["tag"] = args.tag
    print(f"\n=== SUMMARY (DEnsemble + Adaptive K) tag={args.tag} K_base={args.k} D={args.drop} ===")

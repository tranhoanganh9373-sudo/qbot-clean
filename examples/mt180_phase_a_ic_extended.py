"""mt180 Phase A IC 评估 — Extended universe + IS 期 (Discovery, IS only).

跟 `mt180_phase_a_ic.py` 同 schema, 只改两个 dimension:
- universe: CSI300 (~300) → all_no_st (~3792 stocks, strip SH/SZ prefix)
- IS 期: 2017-01-01 ~ 2020-12-31 (4y) → 2014-01-01 ~ 2020-12-31 (7y)

其他参数严格保持(避免重 sweep 引入 OOS-leak 风险):
- horizon = 20d
- 月度横截面 Spearman IC
- ICIR = mean × √12 / std
...
目的: 把 n_months 从 36 提到 60+, 突破历史 4 次 Phase B abort 低样本天花板.

性能: ~10s/indicator × 352 ≈ 56 min 单进程, 并行 8 worker ≈ 8-12 min.

输出 `data_cache/mt180/ic_results_extended.csv`.
"""
...
PREV_IC_FILE = ROOT / "data_cache" / "mt180" / "ic_results.csv"
OUTPUT = ROOT / "data_cache" / "mt180" / "ic_results_extended.csv"
...
MIN_OBS_PER_MONTH = 30
MIN_MONTHS = 24
N_WORKERS = 8  # 留 2 个 core 给 OS / GC
...
def _load_universe() -> list[str]:
    """读 all_no_st.txt, strip SH/SZ 前缀 -> 6 位 code list."""
    codes: list[str] = []
...
def _worker_init(universe_codes: list[str]) -> None:
    """Worker init: load kline + groupby once per process."""
...
def _worker_task(args: tuple[str, str, str, str]) -> dict | None:
    """One indicator -> IC result dict (or None if skipped)."""
...
def _compare_with_prev(df_new: pd.DataFrame) -> str:
    """Compare Top 30 rank between new (extended) and prev (CSI300 4y) results."""
    if not PREV_IC_FILE.exists():
        return "(prev ic_results.csv not found — skip comparison)"
    ...
    prev_top30_ids = set(prev.head(30)["id"])

    new = df_new.copy()
    new["abs_icir"] = new["icir"].abs()
    new = new.sort_values("abs_icir", ascending=False).reset_index(drop=True)
    new["rank_new"] = new.index + 1
    new_top30_ids = set(new.head(30)["id"])

    overlap = prev_top30_ids & new_top30_ids
    only_new = new_top30_ids - prev_top30_ids
    only_prev = prev_top30_ids - new_top30_ids
    ...
    lines: list[str] = []
    lines.append(f"\n## Top 30 rank stability vs prev CSI300/4y\n")
    lines.append(f"- overlap in Top 30: {len(overlap)}/30")
    lines.append(f"- only in new Top 30 (newly emerged): {len(only_new)}")
    lines.append(f"- only in prev Top 30 (lost rank): {len(only_prev)}")
    lines.append("")
    stable = joined[
        joined["rank_new"].le(30)
        & joined["rank_prev"].le(30)
        & joined["rank_delta"].abs().le(10)
    ].sort_values("rank_new")
    if not stable.empty:
        lines.append("### Stable (robust — rank Δ ≤ 10, both in Top 30):\n")
        lines.append(
        ...
        lines.append("")
    dropped = joined[joined["rank_prev"].le(30) & joined["rank_new"].gt(30)].sort_values(
        "rank_new"
    )
    if not dropped.empty:
        lines.append("### Dropped out of Top 30 (was robust, now noise?):\n")
        lines.append(
            dropped[["name", "rank_prev", "rank_new", "icir_prev", "icir"]]
            .head(15)
            .to_markdown(index=False)
        )
        lines.append("")
    new_entered = joined[joined["rank_prev"].gt(30) & joined["rank_new"].le(30)].copy()
    new_entered_missing = joined[joined["rank_prev"].isna() & joined["rank_new"].le(30)]
    if not new_entered.empty or not new_entered_missing.empty:
        lines.append(
            "### Newly entered Top 30 (was low-rank or missing in prev):\n"
        )
        if not new_entered.empty:
            lines.append("**Was in prev results but rank > 30:**\n")
            lines.append(
                new_entered.sort_values("rank_new")[
                    ["name", "rank_prev", "rank_new", "icir_prev", "icir"]
                ]
                .head(15)
                .to_markdown(index=False)
            )
            lines.append("")
        if not new_entered_missing.empty:
            lines.append("**Missing from prev results (new universe / longer IS revealed):**\n")
            lines.append(
                new_entered_missing.sort_values("rank_new")[
                    ["name", "rank_new", "icir"]
                ]
                .head(15)
                .to_markdown(index=False)
...
def main() -> None:
    t0 = time.time()
    print(f"=== mt180 Phase A IC EXTENDED ===")
    print(f"Universe: {UNIVERSE_NAME}  ({UNIVERSE_FILE.name})")
    print(f"IS period: {IS_PERIOD_STR}  (7 years)")
    print(f"Horizon: {HORIZON_DAYS}d, MIN_OBS_PER_MONTH={MIN_OBS_PER_MONTH}, MIN_MONTHS={MIN_MONTHS}")
    print(f"Workers: {N_WORKERS}")
    print()

    universe = _load_universe()
    bk_codes = set(pd.read_parquet(KLINE_FILE, columns=["code"])["code"].unique())
    universe = [c for c in universe if c in bk_codes]
    print(f"Universe size (intersect baidu_kline): {len(universe)}")
    ...
    buckets = [(0, 35), (36, 36), (37, 59), (60, 71), (72, 84)]
    print(f"  bucketed:")
    ...
    print(top30.to_markdown(index=False))

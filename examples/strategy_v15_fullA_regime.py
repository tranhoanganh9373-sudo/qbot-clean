...
def realistic_window(test_month_start, proxy):
...
def main():
    ...
    print(f"[2/3] {len(months)} 月 walk-forward (regime gate enabled)")

    all_rows = []
    for i, m in enumerate(months, 1):
        try:
            res = realistic_window(m, proxy)
            ...
            all_rows.append({"month": m.strftime("%Y-%m"), "abs_ret_%": 0,
                              "avg_picks": 0, "n_days": 0, "regime_days": "",
                              "n_skipped_limit": 0})

    df = pd.DataFrame(all_rows)
    df.to_csv(OUT_DIR / "v15_fullA_regime_stats.csv", index=False)
    ...
    md = [
        "# v15 = v13 全A + regime gate (4 档)",
        "",
        "**Universe**: 全A股 4942 只 (剔除 ST)",
        f"**Period**: {months[0].strftime('%Y-%m')} → {months[-1].strftime('%Y-%m')} "
        f"({len(months)} 月)",
        f"**Capital**: {PORTFOLIO_VALUE:.0f} 元",
        "**Market proxy**: 全A 每日 close 中位数 (等权)",
        f"**Regime rules**: bear<MA200×{BEAR_MA_RATIO} 空仓; vol>{PANIC_VOL_ANN*100:.0f}% 半仓; "
        f"60d<{DRAWDOWN_60D*100:.0f}% 半仓",
        "",
        "## 业绩对比",
        "",
        "| 项 | v13 全A (无 regime) | v15 全A (4 档 regime) |",
        "|----|-------------------:|--------------------:|",
        f"| 月数 | 14 (kill) | {len(months)} |",
        f"| cum %  | -56 | {m['cum_%']:+.2f} |",
        f"| ann %  | -? | {m['ann_%']:+.2f} |",
        f"| sharpe | -? | {m['sharpe']:.2f} |",
        f"| mdd %  | -? | {m['mdd_%']:.2f} |",
        f"| win %  | 36 (5/14) | {m['win_%']:.2f} |",
    ]
    (OUT_DIR / "v15_fullA_regime_report.md").write_text("\n".join(md), encoding="utf-8")
    print("\n输出: v15_fullA_regime_{stats.csv, report.md}")
